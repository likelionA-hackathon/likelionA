...
export function ConnectionsScreen({ workspaceId }: { workspaceId: string }) {
  ...
  const [shareScopes, setShareScopes] = useState({ requirements: true, references: false, decisions: true, notices: true });
  ...
  const load = useCallback(async () => { setError(null); try { const [items, link, me] = await Promise.all([api.connections(workspaceId), api.getLink(workspaceId), api.me()]); setConnections(items); setPartner(link); setWorkspaceName(me.workspaces.find((workspace) => workspace.id === workspaceId)?.name ?? "내 조직"); } catch (caught) { setError(apiErrorMessage(caught, "연결 정보를 불러오지 못했습니다.")); } }, [workspaceId]);
  ...
  function saveShareScopes() { setMessage("공유 범위 UI 설정을 적용했습니다. DB 연동 후 실제 저장됩니다."); }
